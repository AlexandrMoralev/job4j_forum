...
<div class="container mt-3">
    ...
    <form name="login" action="<c:url value='/registration'/>" method="POST">
        ...
        <div class="form-group">
            <label for="passwordDuplicate">Подтверждение пароля</label>
            <input type="password" name="passwordDuplicate" class="form-control" id="passwordDuplicate">
            <div class="alert alert-danger sr-only" role="alert" id="passwordAlert">
                Пароли не совпадают.
            </div>
        </div>
        <button type="submit" id="button" class="btn btn-primary" onclick="return check()">Зарегистрироваться</button>
        <%--
        disabled="disabled" --%>
    </form>
</div>
...
<script>
    ...
    <%--$('#login').focusout(function (){--%>
        ...
        <%--if (inputLogin.length !== 0) {--%>
            <%--$.ajax({--%>
                <%--url: 'http://' + host + ':' + port + '/registration/' +  inputLogin,--%>
                <%--type: 'GET',--%>
                <%--dataType: 'text'--%>
            <%--}).done(function(data) {--%>
                <%--loginAction(data);--%>
    ...
    function check() {
        ...
        var passDuplicate = $('#passwordDuplicate').val();
        var result = true;
        if (pass.length === 0 || pass !== passDuplicate) {
            result = false;
